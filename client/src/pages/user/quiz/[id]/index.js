...
const QuizAttempt = () => {
  ...
  const handleSubmitQuiz = async () => {
    try {
      ...
      const correctAnswers = quiz.questionBank.questions.reduce(
        (acc, question) => {
      ...
      const response = await fetch(
        "http://localhost:4000/api/attempted-quizzes/submit",
        {
          method: "POST",
          headers: {
  ...
  if (
    !quiz ||
    !quiz?.questionBank ||
    !Array.isArray(quiz.questionBank.questions) ||
    quiz.questionBank.questions.length === 0
  ) {
    return (
      <div className="text-center mt-8 text-xl font-semibold text-red-600">
        Quiz data is invalid or empty.
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-gray-100 min-h-screen overflow-hidden">
      <div className="w-full max-w-[1400px] mx-auto bg-white rounded-lg shadow-lg p-4 sm:p-8">
        ...
        <div className="mb-8">
          <h2 className="text-xl font-semibold text-gray-700 mb-4">
            ...
            {quiz.questionBank.questions.length}
          </h2>
