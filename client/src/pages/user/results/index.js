...
const UserResults = () => {
  ...
  const fetchAttemptedQuizzes = async (userId) => {
    try {
      const response = await fetch(
        `http://localhost:4000/api/attempted-quizzes/${userId}`
      );
      if (!response.ok) {
  ...
  const columns = [
    {
      title: "Quiz Title",
      key: "quizName",
      render: (attempt) => attempt?.quiz?.quizName,
    },
    {
      title: "Attempted Questions",
      key: "totalQuestions",
      render: (attempt) => attempt.answers.length,
    },
    {
      title: "Correct Answers",
      key: "correctAnswers",
      render: (attempt) => attempt.score,
    },
    {
      title: "Score",
      key: "score",
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4 w-24 bg-gray-200 rounded-full h-2.5 dark:bg-gray-700 relative">
              <div
                ...
                style={{ width: `${score}%` }}
              ></div>
              <span className="absolute right-0 text-xs font-bold text-gray-800">
                {score.toFixed(2)}%
              </span>
            </div>
          </div>
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto px-4 py-8 bg-gray-50 min-h-screen overflow-hidden">
      ...
      {attempts.length === 0 ? (
        ...
        <div className="bg-white shadow-md border border-gray-300 rounded-lg p-4 overflow-x-auto">
