...
const UserDashboard = () => {
  ...
  const fetchQuizzes = async () => {
    try {
      ...
      const publishedQuizzes = availableData.filter(
        (quiz) => quiz.state === "publish"
      );
      setAvailableQuizzes(publishedQuizzes);

      const attemptedResponse = await fetch(
        `http://localhost:4000/api/attempted-quizzes/${user._id}`
      );
  ...
  const QuizCard = ({ quiz }) => (
    ...
    >
      <h3 className="text-lg font-bold mb-2 text-gray-800">
        {quiz.quizName ? quiz.quizName : quiz.title}
      </h3>
